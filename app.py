...
def generate_pdf_report(summary, ai_summary, shelf_image):
    ...
    pdf.set_xy(0, 8)
    pdf.cell(0, 12, "AI Smart Retail Vision - Executive Report", align="C")
    pdf.ln(22)
    ...
    pdf.multi_cell(0, 6, clean_text_for_pdf(ai_summary))
    pdf.ln(4)
    ...
    pdf.cell(0, 10, "Active Alerts", ln=True)
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(50, 50, 50)
    for _, msg in summary['alerts']:
        pdf.multi_cell(0, 6, f"- {clean_text_for_pdf(msg)}")
    pdf.ln(4)

    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(30, 41, 59)
    pdf.cell(0, 10, "Shelf Stock Visualization", ln=True)
